...
def analize(block, lev)
  ...
  when Declaration
    ...
    p.decl.each do |elem|
      ...
      if $sem[lev][d_name] == nil
        ...
        case elem[1]
        when ID
          $sem[lev][d_name] = Env.new(d_name, :var, type)
          elem[1] = $sem[lev][d_name]
        when Ary
          $sem[lev][d_name] = Env.new(d_name, :var, [:array, type, elem[1].size])
          elem[1] = $sem[lev][d_name]
        end
      else
  ...
  when Func_proto
    ...
    if  $sem[lev][p_name] == nil
      $sem[0][p_name] = Env.new(p_name, :proto, type)
      p.name = $sem[0][p_name]
  ...
  when Func_def
    ...
    if $sem[0][f_name] ==  nil || ($sem[0][f_name].kind == :proto && $sem[0][f_name].type == type)
      $sem[0][f_name] = Env.new(f_name, :fun, type)
      p.name = $sem[0][f_name]
      $ftype = type[0]
    else
    ...
    p.arg.each do |a|
      ...
      if $sem[1][a_name] == nil
        $sem[1][a_name] = Env.new(a_name, :parm, type)
        a[1] = $sem[1][a_name]
      else
      ...
      if p.arg == :val
        ...
        if p.arg == :void
          ...
          if env.type[1].size != p.arg.size
            puts  "#{p.pos} error : function parameter is defferent."
            exit 1
          else
